...
pts.extcvm$prop_t <- sqrt(pts.extcvm$prop) #clay ## UPDATE: transform if needed else just create new version of prop
...
pts.extpcv$pcvpred_bt <- (pts.extpcv$pcvpred)^2 #clay ## ALWAYS Update with backtransformation (if needed)
...
## RPI
pts.extpcv$prop_bt <- (pts.extpcv$prop_t)^2 #clay # UPDATE: backtransform if necessary. Used for PICP and to characterize backtransformation bias
...
gplt.dcm.2D.CV <- ggplot(data=pts.extpcv, aes(prop_t, pcvpred_bt)) +
  stat_binhex(bins = 30) + geom_abline(intercept = 0, slope = 1,lwd=1)  + xlim(0,10) + ylim(0,10) +
