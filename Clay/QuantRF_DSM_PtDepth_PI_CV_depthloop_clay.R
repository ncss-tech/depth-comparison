...
pts.extcvm$prop_t <- sqrt(pts.extcvm$prop) #clay # transform if needed else just create new version of prop
...
gplt.dcm.2D.CV <- ggplot(data=pts.extpcv, aes(prop_t, pcvpred)) +
  stat_binhex(bins = 30) + geom_abline(intercept = 0, slope = 1,lwd=1)  + xlim(0,9) + ylim(0,9) +
