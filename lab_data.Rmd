...
h_df_long <- within(h_df_long, {
  # parse method
  method = ifelse(grepl("sp", variable), "spline point",      NA)
  method = ifelse(grepl("dp", variable), "depth point",       method)
  method = ifelse(grepl("si", variable), "spline interval",   method)
  method = ifelse(grepl("wa", variable), "weighted interval", method)
...
aggregate(value ~ method, data = h_df_long[h_df_long$var == "clay" &  h_df_long$value > -1, ], function(x) round(summary(x)), na.action = na.pass)

aggregate(value ~ method, data = h_df_long[h_df_long$var == "clay" &  h_df_long$value > -1, ], function(x) var(x, na.rm = TRUE))

h_df_long %>%
  filter(var == "clay" & value > 0 & pedon_key %in% s_ucrb$pedon_key) %>%
